 ...
 * @author Kurt De Vos
 */
public class SynapseMeasuresCellCntrMarkerVector extends Vector{
    private int type;
    private Color color;
    /** Creates a new instance of MarkerVector */
    public SynapseMeasuresCellCntrMarkerVector(int type) {
        super();
        this.type=type;
        color = createColor(type);
    }
    public void addMarker(SynapseMeasuresCellCntrMarker marker){
        add(marker);
    }
    
    public SynapseMeasuresCellCntrMarker getMarker(int n){
        return (SynapseMeasuresCellCntrMarker)get(n);
    }
    public int getVectorIndex(SynapseMeasuresCellCntrMarker marker){
    ...
    private boolean isCloser(SynapseMeasuresCellCntrMarker m1,SynapseMeasuresCellCntrMarker m2, Point p){
    ...
    public SynapseMeasuresCellCntrMarker getMarkerFromPosition(Point p, int sliceIndex){
        Vector v = new Vector();
        ListIterator it = this.listIterator();
        while(it.hasNext()){
            SynapseMeasuresCellCntrMarker m = (SynapseMeasuresCellCntrMarker)it.next();
            if (m.getZ()==sliceIndex){
                v.add(m);
            }
        }
        SynapseMeasuresCellCntrMarker currentsmallest = (SynapseMeasuresCellCntrMarker)v.get(0);
        for (int i=1; i<v.size(); i++){
            SynapseMeasuresCellCntrMarker m2 = (SynapseMeasuresCellCntrMarker)v.get(i);
